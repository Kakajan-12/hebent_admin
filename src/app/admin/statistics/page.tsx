...
const StatisticsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await get<StatisticItem[]>("/api/statistics");
        setItems(data);
      } catch (err) {
        console.error(err);
    ...
    try {
      await Promise.all(
        Array.from(selectedIds).map((id) =>
          deleteRequest<void>(`/api/statistics/${id}`),
        ),
      );
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/admin/statistics/add-statistics"
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex flex-wrap items-center justify-center gap-2">
                          <Link
                            href={`/admin/statistics/edit-statistics/${row.id}`}
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-md">
              <h2 className="mb-4 text-xl font-semibold">Remove statistics?</h2>
              <p className="mb-6">
                Are you sure you want to delete {selectedIds.size} statistic
