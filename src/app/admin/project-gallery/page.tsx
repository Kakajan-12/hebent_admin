...
const getProjectName = (project?: Project) => {
  if (!project) return "";
  return (
    stripHtml(project.title_en) ||
    stripHtml(project.title_ru) ||
    stripHtml(project.title_tk)
  );
};
...
const ProjectGalleryPage = () => {
  ...
  const filteredGallery = useMemo(() => {
    if (!selectedProjectId) return gallery;
    return gallery.filter(
      (item) => item.project_id === Number(selectedProjectId),
    );
  ...
  const fetchGalleryList = useCallback(async () => {
    const galleryData = await api.get<GalleryItem[]>("/api/project-gallery");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedProjectId || files.length === 0) {
      setError("Please select project and image files.");
      return;
    }
    ...
    try {
      ...
      const createdItems = await Promise.all(
        files.map((file) => {
          ...
          return api.post<GalleryItem>("/api/project-gallery", formData);
        }),
      );
  ...
  const handleDelete = async (id: number) => {
    try {
      setError("");
      setDeletingId(id);
      await api.delete<void>(`/api/project-gallery/${id}`);
      await fetchGalleryList();
    } catch (err) {
            ...
            <tbody>
              {loading ? (
                ...
                filteredGallery.map((item) => {
                  ...
                  const projectName =
                    getProjectName(project) || `Project #${item.project_id}`;
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex flex-wrap items-center justify-center gap-2">
                          <Link
                            href={`/admin/project-gallery/edit-gallery/${item.id}`}
