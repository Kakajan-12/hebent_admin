...
const TestimonialsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await get<TestimonialItem[]>("/api/testimonials");
        setItems(data);
      } catch (err) {
        console.error(err);
    ...
    try {
      await Promise.all(
        Array.from(selectedIds).map((id) =>
          deleteRequest<void>(`/api/testimonials/${id}`),
        ),
      );
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/admin/testimonials/add-testimonial"
                ...
                items.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="max-w-md border-r border-dashed border-[#D8D8D8] px-4 py-4 text-center">
                        <div
                          className="line-clamp-3 text-left [&_p]:my-0"
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex flex-wrap items-center justify-center gap-2">
                          <Link
                            href={`/admin/testimonials/view-testimonial/${row.id}`}
                          ...
                          <Link
                            href={`/admin/testimonials/edit-testimonial/${row.id}`}
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-md">
              ...
              <p className="mb-6">
                Are you sure you want to delete {selectedIds.size} testimonial
